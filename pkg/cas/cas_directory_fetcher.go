...
func NewCASDirectoryFetcher(directoryReader reader.Reader[*remoteexecution.Directory], treeReader cas.StreamReader, maximumDirectorySizeBytes, maximumTreeSizeBytes int64) DirectoryFetcher {
...
func (df *casDirectoryFetcher) GetDirectory(ctx context.Context, directoryDigest digest.Digest) (*remoteexecution.Directory, error) {
	m, err := df.directoryReader.Read(ctx, directoryDigest)
	if err != nil {
		return nil, err
	}
	return m, nil
