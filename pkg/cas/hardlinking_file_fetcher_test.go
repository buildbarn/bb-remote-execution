...
func TestHardlinkingFileFetcher(t *testing.T) {
	...
	baseFileFetcher.EXPECT().GetFile(ctx, blobDigest1, buildDirectory, "hello.txt", false).
		Return(status.Error(codes.Internal, "Server not reachable"))
	require.Equal(
		t,
		status.Error(codes.Internal, "Server not reachable"),
		fileFetcher.GetFile(ctx, blobDigest1, buildDirectory, "hello.txt", false))

	// Failing to link the file into the cache should not cause the
	// file to become cached immediately.
	baseFileFetcher.EXPECT().GetFile(ctx, blobDigest1, buildDirectory, "hello.txt", false)
	buildDirectory.EXPECT().Link("hello.txt", cacheDirectory, "8b1a9953c4611296a827abf8c47804d7-5-x").
		Return(syscall.EIO)
	require.Equal(
		t,
	...
	// opposite direction.
	cacheDirectory.EXPECT().Link("8b1a9953c4611296a827abf8c47804d7-5-x", buildDirectory, "hello.txt")
	require.NoError(
		t,
		fileFetcher.GetFile(ctx, blobDigest1, buildDirectory, "hello.txt", false))
	...
	baseFileFetcher.EXPECT().GetFile(ctx, blobDigest2, buildDirectory, "goodbye.txt", false)
	cacheDirectory.EXPECT().Remove("8b1a9953c4611296a827abf8c47804d7-5-x").
		Return(syscall.EIO)
	require.Equal(
		t,
		status.Error(codes.Internal, "Failed to remove cached file \"8b1a9953c4611296a827abf8c47804d7-5-x\": input/output error"),
		fileFetcher.GetFile(ctx, blobDigest2, buildDirectory, "goodbye.txt", false))
	...
	cacheDirectory.EXPECT().Remove("8b1a9953c4611296a827abf8c47804d7-5-x").
		Return(syscall.ENOENT)
	buildDirectory.EXPECT().Link("goodbye.txt", cacheDirectory, "6fc422233a40a75a1f028e11c3cd1140-7-x")
	require.NoError(
		t,
		fileFetcher.GetFile(ctx, blobDigest2, buildDirectory, "goodbye.txt", false))
}
