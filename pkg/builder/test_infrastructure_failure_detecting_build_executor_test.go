...
func TestTestInfrastructureFailureDetectingBuildExecutor(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		testutil.RequireEqualProto(t, failedResponse, buildExecutor.Execute(ctx, filePool, instanceName, request, metadata))
	}
	...
	baseBuildExecutor.EXPECT().Execute(ctx, filePool, instanceName, request, metadata).Return(successfulResponse)
	testutil.RequireEqualProto(t, successfulResponse, buildExecutor.Execute(ctx, filePool, instanceName, request, metadata))
	...
	}, buildExecutor.Execute(ctx, filePool, instanceName, request, metadata))
