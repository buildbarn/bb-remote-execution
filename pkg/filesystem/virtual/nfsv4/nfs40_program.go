...
func (s *compoundState) txLockCommon(args *nfsv4.Lock4args, lofs *nfs40LockOwnerFileState) nfsv4.Lock4res {
	lockCount, res := lofs.openOwnerFile.openedFile.Lock(&lofs.lockOwner.owner, args.Offset, args.Length, args.Locktype)
	if res != nil {
		return res
	}
	lofs.lockCount += lockCount
	if lofs.lockCount < 0 {
...
func (s *compoundState) opLockt(args *nfsv4.Lockt4args) nfsv4.Lockt4res {
	...
	if los, ok := confirmedClient.lockOwners[string(args.Owner.Owner)]; ok {
		owner = &los.owner
	}
	...
	case *nfsv4.OpenClaim4_CLAIM_NULL:
		...
		if ok {
			...
			oofs = &nfs40OpenOwnerFileState{
				openOwner:      oos,
				openedFile:     p.openedFilesPool.Open(handle, leaf),
...
func (oofs *nfs40OpenOwnerFileState) removeFinalize(p *nfs40Program) {
	...
	handleKey := string(oofs.openedFile.GetHandle())
...
// nfs40OpenOwnerState stores information on a single lock-owner, which
...
func (lofs *nfs40LockOwnerFileState) remove(p *nfs40Program, ll *leavesToClose) {
	...
	// associated with it.
	if len(los.files) == 0 {
		delete(los.confirmedClient.lockOwners, string(los.owner.Owner))
	}
}
...
func attrRequestToAttributesMask(attrRequest nfsv4.Bitmap4) virtual.AttributesMask {
	...
	if len(attrRequest) > 0 {
		...
		if f&uint32(1<<nfsv4.FATTR4_SIZE) != 0 {
			attributesMask |= virtual.AttributesMaskSizeBytes | virtual.AttributesMaskSymlinkTarget
		}
...
// NFSv4 equivalent.
func pathParserToLinktext4(pathParser path.Parser, pathFormat path.Format) (nfsv4.Linktext4, bool) {
	pathBuilder, scopeWalker := path.EmptyBuilder.Join(path.VoidScopeWalker)
	if err := path.Resolve(pathParser, scopeWalker); err != nil {
		return nil, false
	}
	pathString, err := pathFormat.GetString(pathBuilder)
	if err != nil {
		return nil, false
	}
	return nfsv4.Linktext4(pathString), true
}
