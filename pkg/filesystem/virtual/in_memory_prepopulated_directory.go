...
func normalizeComponent(c path.Component, caseSensitive bool) normalizedComponent {
	if caseSensitive {
...
func (n normalizedComponent) String() string {
	return n.string
}
...
func (c *inMemoryFilesystem) normalizeComponent(name path.Component) normalizedComponent {
	return normalizeComponent(name, c.caseSensitive)
}
...
func (c *inMemoryDirectoryContents) createChildren(subtree *inMemorySubtree, children map[path.Component]InitialChild) {
	...
	for _, name := range namesList {
		normalizedName := subtree.filesystem.normalizeComponent(name)
...
func NewInMemoryPrepopulatedDirectory(fileAllocator FileAllocator, symlinkFactory SymlinkFactory, errorLogger util.ErrorLogger, handleAllocator StatefulHandleAllocator, initialContentsSorter Sorter, hiddenFilesMatcher StringMatcher, clock clock.Clock, caseSensitive bool, defaultAttributesSetter DefaultAttributesSetter) PrepopulatedDirectory {
	subtree := &inMemorySubtree{
		filesystem: &inMemoryFilesystem{
			symlinkFactory:          symlinkFactory,
...
func (i *inMemoryPrepopulatedDirectory) LookupChild(name path.Component) (PrepopulatedDirectoryChild, error) {
	...
	normalizedName := i.subtree.filesystem.normalizeComponent(name)
...
func (i *inMemoryPrepopulatedDirectory) VirtualRename(oldName path.Component, newDirectory Directory, newName path.Component) (ChangeInfo, ChangeInfo, Status) {
	...
	normalizedOldName := iOld.subtree.filesystem.normalizeComponent(oldName)
	normalizedNewName := i.subtree.filesystem.normalizeComponent(newName)
...
func (i *inMemoryPrepopulatedDirectory) VirtualSymlink(ctx context.Context, pointedTo []byte, linkName path.Component, requested AttributesMask, out *Attributes) (Leaf, ChangeInfo, Status) {
	...
	normalizedLinkName := i.subtree.filesystem.normalizeComponent(linkName)
