...
func TestCorrelatedInvocationsIDInvocationKeyExtractor(t *testing.T) {
	ctx := context.Background()
	digestFunction := digest.MustNewFunction("hello", remoteexecution.DigestFunction_SHA256)

	key, err := invocation.CorrelatedInvocationsIDKeyExtractor.ExtractKey(ctx, digestFunction, &remoteexecution.Action{}, &remoteexecution.RequestMetadata{
	...
	id, err := anypb.New(&remoteexecution.RequestMetadata{
		CorrelatedInvocationsId: "92d71492-175e-40ba-9c8c-3b5d3b9a6808",
	})
	require.NoError(t, err)
