...
func main() {
	program.RunMain(func(ctx context.Context, siblingsGroup, dependenciesGroup program.Group) error {
		...
		blockDevice, allocator, sectorSize, err := pool.NewFilepoolParametersFromConfiguration(configuration.FilePool)
		if err != nil {
		...
		for _, buildDirectoryConfiguration := range configuration.BuildDirectories {
			...
			for _, runnerConfiguration := range buildDirectoryConfiguration.Runners {
				...
				for threadID := uint64(0); threadID < runnerConfiguration.Concurrency; threadID++ {
					...
					maximumSectors := runnerConfiguration.MaximumFilePoolSizeBytes / int64(sectorSize)
					// TODO: Does the prometheus metrics construction
					// support being registered multiple times?
					runnerFilePool := pool.NewQuotaEnforcingFilePool(
						pool.NewMetricsFilePool(
							pool.NewBlockDeviceBackedFilePool(
								blockDevice,
								re_filesystem.NewQuotaEnforcingSectorAllocator(
									allocator,
									maximumSectors,
								),
								sectorSize,
